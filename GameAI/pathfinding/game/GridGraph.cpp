...
void GridGraph::VisualizeNodeCosts()
{
	...
	while (pCurrentNode != NULL)
	{
		...
		gpGame->getGraphicsSystem()->draw(*gpGame->getGraphicsSystem()->getBackBuffer(),*gpGame->getSpriteManager()->getSprite(NODE_DIRECTION_SPRITE_ID), ulPos.getX() + 12, ulPos.getY() + 5, nodeDirection);
		gpGame->getGraphicsSystem()->writeText(*gpGame->getGraphicsSystem()->getBackBuffer(), *gpGame->getFont(), ulPos.getX(), ulPos.getY(), std::to_string(nodeCost).c_str(), Color(200, 200, 200));
		i++;
